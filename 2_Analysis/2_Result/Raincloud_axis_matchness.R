...
df.agg2 <- df.agg2 %>%
  dplyr::mutate(
    ...
    Seq_nudge = dplyr::case_when(
      valence == "Good" & matchness == "match" ~ 1.3,
      valence == "Good" & matchness == "mismatch" ~ 4.9,
      valence == "Neutral" & matchness == "match" ~ 1.5,
      valence == "Neutral" & matchness == "mismatch" ~ 4.7,
      valence == "Bad" & matchness == "match" ~ 1.7,
      valence == "Bad" & matchness == "mismatch" ~ 4.5,
    ),
    Con_j = jitter(Seq_nudge, amount = 0.09)
  )
...
df_avg2 <- df.agg2 %>%
  ...
  dplyr::mutate(
    ...
    Seq_nudge = dplyr::case_when(
      valence == "Good" & matchness == "match" ~ 0.6,
      valence == "Good" & matchness == "mismatch" ~ 5.4,
      valence == "Neutral" & matchness == "match" ~ 0.8,
      valence == "Neutral" & matchness == "mismatch" ~ 5.2,
      valence == "Bad" & matchness == "match" ~ 1,
      valence == "Bad" & matchness == "mismatch" ~ 5.0,
    )
  ) %>%
  dplyr::ungroup()
...
df.agg2$Sequence <- factor(df.agg2$Sequence, levels = c("Word first", "Image first", "Simultaneous"))
df_avg2$Sequence <- factor(df_avg2$Sequence, levels = c("Word first", "Image first", "Simultaneous"))


ggplot(data = df.agg2, aes(y = rt_m)) +
  geom_boxplot(data = df.agg2 %>% dplyr::filter(Seq_posi == 1), aes(x = Seq_nudge, y = rt_m, fill = valence), outlier.shape = NA, width = 0.1) +
  geom_boxplot(data = df.agg2 %>% dplyr::filter(Seq_posi == 5), aes(x = Seq_nudge, y = rt_m, fill = valence), outlier.shape = NA, width = 0.1) +
  geom_point(data = df.agg2 %>% dplyr::filter(Seq_posi == 1), aes(x = Con_j, y = rt_m, color = valence), alpha = 0.3) +
  geom_point(data = df.agg2 %>% dplyr::filter(Seq_posi == 5), aes(x = Con_j, y = rt_m, color = valence), alpha = 0.3) +
  geom_line(data = df.agg2 %>% filter(valence == "Good"), aes(x = Con_j, group = subj_idx), color = "lightgrey", alpha = 0.3) +
  geom_line(data = df.agg2 %>% filter(valence == "Neutral"), aes(x = Con_j, group = subj_idx), color = "lightgrey", alpha = 0.3) +
  geom_line(data = df.agg2 %>% filter(valence == "Bad"), aes(x = Con_j, group = subj_idx), color = "lightgrey", alpha = 0.3) +
  gghalves::geom_half_violin(
    data = df.agg2 %>% dplyr::filter(Seq_posi == 1), aes(x = Seq_posi, y = rt_m, fill = valence), side = "l", position = position_nudge(x = -.4),
    trim = FALSE, alpha = 0.8, color = NA
  ) +
  gghalves::geom_half_violin(
    data = df.agg2 %>% dplyr::filter(Seq_posi == 5), aes(x = Seq_posi, y = rt_m, fill = valence), side = "r", position = position_nudge(x = .6),
    trim = FALSE, alpha = 0.8, color = NA
  ) +
  geom_pointrange(
    data = df_avg2 %>% dplyr::filter(Seq_posi == 1), aes(
      x = Seq_nudge, y = meanrt,
      ymin = meanrt - se,
      ymax = meanrt + se,
      color = valence, fill = valence
  ...
  geom_pointrange(
    data = df_avg2 %>% dplyr::filter(Seq_posi == 5), aes(
      x = Seq_nudge, y = meanrt,
      ymin = meanrt - se,
      ymax = meanrt + se,
      color = valence, fill = valence
...
ggsave("matchness_axis2.pdf", width = 15, height = 7, dpi = 300)
getwd()
