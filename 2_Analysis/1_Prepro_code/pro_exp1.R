...
here::here()
path1 <- here("1_Data", "1_Raw_data", "1_exp1_data")
...
for (i in seq_along(filenames)){
  ...
  data[[i]] <- data[[i]] %>% 
    select(subj_idx, gender, year, education, dist, trial_type, rt, response, 
...
outpath1 <- here("1_Data", "2_Postpro_data")
write_csv(df_exp1, paste(outpath1, "Exp1_postpro.csv", sep = "/"))
